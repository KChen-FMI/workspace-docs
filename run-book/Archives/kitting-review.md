...
## 3. To-Be（改修案 v2：折衷案＋運用強化）

### 基本方針

* **待機PCは常時2台**のみ（増やさない）
* Autopilotは**前準備工程として放置**
* 成否は**仕組みで強制**（善意に依存しない）
...
subgraph 常時運用[常時運用（オーダー非依存）]
    A1[待機PC 2台を維持する]
    A1 --> A2[HWID登録]
    A2 --> A3[Autopilot / Intune 割当]
    A3 --> A4[デバイス共通ソフト配布]
    A4 --> A5[前準備完了（待機2台）
電源OFF保管]
end
...
%%==============================
%% EVENT: オーダー（開始ではなく割込み）
%%==============================
A5 -. 割込みイベント .-> E0((EVENT: オーダー発生))

subgraph オーダー後[オーダー発生後（イベント処理）]
    E0 --> B2[待機PCを1台引当]
    B2 --> B3[ユーザ確定]
    B3 --> B4[初回サインイン]
    B4 --> B5[個人設定]
